...
/*------------------------------------------------------------------------------------------*/        
/* Addresses to scan */
static unsigned short normal_i2c[2] = {0x3a,I2C_CLIENT_END};
...
static int ctp_detect(struct i2c_client *client, struct i2c_board_info *info)
{
	struct i2c_adapter *adapter = client->adapter;
			...
			printk("I2C connection might be sonmthing wrong!\n");
			return 0;
		}else{
			printk("-----------------twi_id != adapter->nr--------------------------------------------------\n");
			return -ENODEV;
}
...
static void fts_get_upgrade_info(struct Upgrade_Info *upgrade_info)
{
	...
	case 0x55:    //IC_FT5X06:
		...
		break;
	case 0x08:    //IC_FT5606»òÕßIC_FT5506
...
{ 
	...
	//
	normal_i2c[0]=config_info.twi_addr;
	printk("*** twi_addr in config is:%d ,set to normal_i2c arr\n",config_info.twi_addr);
	/*TP上电函数*/

	if (IS_ERR(ldo)) {
		printk("glsX680 get regulator error!! \n");
		return -1;
	} else {
